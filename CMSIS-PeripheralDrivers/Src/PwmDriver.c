...
TIM_TypeDef	*ptrPwmUsed;

/**/
void pwm_Config(PWM_Handler_t *ptrPwmHandler){

	// Guardamos una referencia al periferico que estamos utilizando...
	ptrPwmUsed = ptrPwmHandler->ptrTIMx;
	...
	// Configurar el registro que nos controla el modo up
	ptrPwmUsed->CR1 &= ~TIM_CR1_DIR;
	//Reiniciamos el registro counter
	ptrPwmUsed->CNT = ~TIM_CNT_CNT;
	// Auto-reload preload enable
	ptrPwmUsed->CR1 |= TIM_CR1_ARPE;
	...
	case PWM_CHANNEL_1:{
		...
		ptrPwmUsed->CCMR1 &= ~TIM_CCMR1_CC1S;

		// Configuramos el canal como PWM1
		ptrPwmUsed->CCMR1 |= (0x6UL << TIM_CCMR1_OC1M_Pos);

		// Polarity active high
		ptrPwmUsed->CCER &= ~TIM_CCER_CC1P;

		// Activamos la funcionalidad de pre-load
		ptrPwmUsed->CCMR1 |= TIM_CCMR1_OC1PE;
	...
	case PWM_CHANNEL_2:{
		// Seleccionamos como salida el canal
		ptrPwmUsed->CCMR1 &= ~TIM_CCMR1_CC2S;

		/// Configuramos el canal como PWM1
		ptrPwmUsed->CCMR1 |= (0x6UL << TIM_CCMR1_OC2M_Pos);

		// Polarity active high
		ptrPwmUsed->CCER &= ~TIM_CCER_CC2P;

		// Activamos la funcionalidad de pre-load
		ptrPwmUsed->CCMR1 |= TIM_CCMR1_OC2PE;
		break;
	}

	case PWM_CHANNEL_3:{
		// Seleccionamos como salida el canal
		ptrPwmUsed->CCMR2 &= ~TIM_CCMR2_CC3S;

		// Configuramos el canal como PWM
		ptrPwmUsed->CCMR2 |= (0x6UL << TIM_CCMR2_OC3M_Pos);

		// Polarity active high
		ptrPwmUsed->CCER &= ~TIM_CCER_CC3P;

		// Activamos la funcionalidad de pre-load
		ptrPwmUsed->CCMR2 |= TIM_CCMR2_OC3PE;
		break;
	}

	case PWM_CHANNEL_4:{
		// Seleccionamos como salida el canal
		ptrPwmUsed->CCMR2 &= ~TIM_CCMR2_CC4S;

		// Configuramos el canal como PWM
		ptrPwmUsed->CCMR2 |= (0x6UL << TIM_CCMR2_OC4M_Pos);

		// Polarity active high
		ptrPwmUsed->CCER &= ~TIM_CCER_CC4P;

		// Activamos la funcionalidad de pre-load
		ptrPwmUsed->CCMR2 |= TIM_CCMR2_OC4PE;
		break;
	}
...
void enableOutput(PWM_Handler_t *ptrPwmHandler) {
	...
	case PWM_CHANNEL_1: {
		...
		ptrPwmUsed->CCER |= TIM_CCER_CC1E;
		break;
	}

	case PWM_CHANNEL_2: {
		// Activamos la salida del canal 2
		ptrPwmUsed->CCER |= TIM_CCER_CC2E;
		break;
	}

	case PWM_CHANNEL_3: {
		// Activamos la salida del canal3
		ptrPwmUsed->CCER |= TIM_CCER_CC3E;
		break;
	}
	case PWM_CHANNEL_4: {
		// Activamos la salida del canal 1
		ptrPwmUsed->CCER |= TIM_CCER_CC4E;
		break;
	}
